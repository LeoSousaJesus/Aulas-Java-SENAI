...
public class UsuarioDao {
    ...
    // Novo método que impede duplicidade de e-mails em novos cadastros
    public boolean salvarOuAtualizarUsuario(Usuario usuario, boolean isNovoCadastro) {
        Usuario existente = buscarUsuarioPorEmail(usuario.getEmailUsuario());

        if (isNovoCadastro) {
            if (existente != null) {
                return false; // Já existe e-mail no cadastro novo
            }
            return salvarUsuario(usuario);
        } else {
            // Verifica se o e-mail pertence a outro usuário (evita troca de email duplicado)
            if (existente != null && existente.getIdUsuario() != usuario.getIdUsuario()) {
                return false; // Outro usuário já tem esse e-mail
            }
